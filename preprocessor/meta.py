def print_current_progress_to_console(current_progress, maximum_progress, status_message=''):
    """
    # Adapted from https://gist.github.com/vladignatyev/06860ec2040cb497f0f3#file-progress-py-L18
    # For the original code:
    # The MIT License (MIT)
    # Copyright (c) 2016 Vladimir Ignatev

    Examples:
        >>> from preprocessor.Text_File import Text_File
        >>> from preprocessor.meta import print_current_progress_to_console
        >>> import time

        >>> my_file = Text_File('C://Users//Clokman//Google Drive//__Projects__//Code//KFIR//preprocessor//test_data//blazegraph_output_1000.csv')
        >>> with open(my_file.input_file_path, encoding='utf8') as file:
        ...    current_progress = 0
        ...    maximum_progress = my_file.get_no_of_lines_in_file()
        ...    for each_line in file:
        ...        #time.sleep(0.5)  # uncomment for waiting for x seconds (for emulating doing something)
        ...
        ...        print_current_progress_to_console(current_progress, maximum_progress, 'Iterating')
        ...        current_progress += 1
        [------------------------------------------------------------] 0.0% ...Iterating
        [------------------------------------------------------------] 0.1% ...Iterating
        [------------------------------------------------------------] 0.2% ...Iterating
        [------------------------------------------------------------] 0.3% ...Iterating
        [------------------------------------------------------------] 0.4% ...Iterating
        [------------------------------------------------------------] 0.5% ...Iterating
        [------------------------------------------------------------] 0.6% ...Iterating
        [------------------------------------------------------------] 0.7% ...Iterating
        [------------------------------------------------------------] 0.8% ...Iterating
        [=-----------------------------------------------------------] 0.9% ...Iterating
        [=-----------------------------------------------------------] 1.0% ...Iterating
        [=-----------------------------------------------------------] 1.1% ...Iterating
        [=-----------------------------------------------------------] 1.2% ...Iterating
        [=-----------------------------------------------------------] 1.3% ...Iterating
        [=-----------------------------------------------------------] 1.4% ...Iterating
        [=-----------------------------------------------------------] 1.5% ...Iterating
        [=-----------------------------------------------------------] 1.6% ...Iterating
        [=-----------------------------------------------------------] 1.7% ...Iterating
        [=-----------------------------------------------------------] 1.8% ...Iterating
        [=-----------------------------------------------------------] 1.9% ...Iterating
        [=-----------------------------------------------------------] 2.0% ...Iterating
        [=-----------------------------------------------------------] 2.1% ...Iterating
        [=-----------------------------------------------------------] 2.2% ...Iterating
        [=-----------------------------------------------------------] 2.3% ...Iterating
        [=-----------------------------------------------------------] 2.4% ...Iterating
        [=-----------------------------------------------------------] 2.5% ...Iterating
        [==----------------------------------------------------------] 2.6% ...Iterating
        [==----------------------------------------------------------] 2.7% ...Iterating
        [==----------------------------------------------------------] 2.8% ...Iterating
        [==----------------------------------------------------------] 2.9% ...Iterating
        [==----------------------------------------------------------] 3.0% ...Iterating
        [==----------------------------------------------------------] 3.1% ...Iterating
        [==----------------------------------------------------------] 3.2% ...Iterating
        [==----------------------------------------------------------] 3.3% ...Iterating
        [==----------------------------------------------------------] 3.4% ...Iterating
        [==----------------------------------------------------------] 3.5% ...Iterating
        [==----------------------------------------------------------] 3.6% ...Iterating
        [==----------------------------------------------------------] 3.7% ...Iterating
        [==----------------------------------------------------------] 3.8% ...Iterating
        [==----------------------------------------------------------] 3.9% ...Iterating
        [==----------------------------------------------------------] 4.0% ...Iterating
        [==----------------------------------------------------------] 4.1% ...Iterating
        [===---------------------------------------------------------] 4.2% ...Iterating
        [===---------------------------------------------------------] 4.3% ...Iterating
        [===---------------------------------------------------------] 4.4% ...Iterating
        [===---------------------------------------------------------] 4.5% ...Iterating
        [===---------------------------------------------------------] 4.6% ...Iterating
        [===---------------------------------------------------------] 4.7% ...Iterating
        [===---------------------------------------------------------] 4.8% ...Iterating
        [===---------------------------------------------------------] 4.9% ...Iterating
        [===---------------------------------------------------------] 5.0% ...Iterating
        [===---------------------------------------------------------] 5.1% ...Iterating
        [===---------------------------------------------------------] 5.2% ...Iterating
        [===---------------------------------------------------------] 5.3% ...Iterating
        [===---------------------------------------------------------] 5.4% ...Iterating
        [===---------------------------------------------------------] 5.5% ...Iterating
        [===---------------------------------------------------------] 5.6% ...Iterating
        [===---------------------------------------------------------] 5.7% ...Iterating
        [===---------------------------------------------------------] 5.8% ...Iterating
        [====--------------------------------------------------------] 5.9% ...Iterating
        [====--------------------------------------------------------] 6.0% ...Iterating
        [====--------------------------------------------------------] 6.1% ...Iterating
        [====--------------------------------------------------------] 6.2% ...Iterating
        [====--------------------------------------------------------] 6.3% ...Iterating
        [====--------------------------------------------------------] 6.4% ...Iterating
        [====--------------------------------------------------------] 6.5% ...Iterating
        [====--------------------------------------------------------] 6.6% ...Iterating
        [====--------------------------------------------------------] 6.7% ...Iterating
        [====--------------------------------------------------------] 6.8% ...Iterating
        [====--------------------------------------------------------] 6.9% ...Iterating
        [====--------------------------------------------------------] 7.0% ...Iterating
        [====--------------------------------------------------------] 7.1% ...Iterating
        [====--------------------------------------------------------] 7.2% ...Iterating
        [====--------------------------------------------------------] 7.3% ...Iterating
        [====--------------------------------------------------------] 7.4% ...Iterating
        [====--------------------------------------------------------] 7.5% ...Iterating
        [=====-------------------------------------------------------] 7.6% ...Iterating
        [=====-------------------------------------------------------] 7.7% ...Iterating
        [=====-------------------------------------------------------] 7.8% ...Iterating
        [=====-------------------------------------------------------] 7.9% ...Iterating
        [=====-------------------------------------------------------] 8.0% ...Iterating
        [=====-------------------------------------------------------] 8.1% ...Iterating
        [=====-------------------------------------------------------] 8.2% ...Iterating
        [=====-------------------------------------------------------] 8.3% ...Iterating
        [=====-------------------------------------------------------] 8.4% ...Iterating
        [=====-------------------------------------------------------] 8.5% ...Iterating
        [=====-------------------------------------------------------] 8.6% ...Iterating
        [=====-------------------------------------------------------] 8.7% ...Iterating
        [=====-------------------------------------------------------] 8.8% ...Iterating
        [=====-------------------------------------------------------] 8.9% ...Iterating
        [=====-------------------------------------------------------] 9.0% ...Iterating
        [=====-------------------------------------------------------] 9.1% ...Iterating
        [======------------------------------------------------------] 9.2% ...Iterating
        [======------------------------------------------------------] 9.3% ...Iterating
        [======------------------------------------------------------] 9.4% ...Iterating
        [======------------------------------------------------------] 9.5% ...Iterating
        [======------------------------------------------------------] 9.6% ...Iterating
        [======------------------------------------------------------] 9.7% ...Iterating
        [======------------------------------------------------------] 9.8% ...Iterating
        [======------------------------------------------------------] 9.9% ...Iterating
        [======------------------------------------------------------] 10.0% ...Iterating
        [======------------------------------------------------------] 10.1% ...Iterating
        [======------------------------------------------------------] 10.2% ...Iterating
        [======------------------------------------------------------] 10.3% ...Iterating
        [======------------------------------------------------------] 10.4% ...Iterating
        [======------------------------------------------------------] 10.5% ...Iterating
        [======------------------------------------------------------] 10.6% ...Iterating
        [======------------------------------------------------------] 10.7% ...Iterating
        [======------------------------------------------------------] 10.8% ...Iterating
        [=======-----------------------------------------------------] 10.9% ...Iterating
        [=======-----------------------------------------------------] 11.0% ...Iterating
        [=======-----------------------------------------------------] 11.1% ...Iterating
        [=======-----------------------------------------------------] 11.2% ...Iterating
        [=======-----------------------------------------------------] 11.3% ...Iterating
        [=======-----------------------------------------------------] 11.4% ...Iterating
        [=======-----------------------------------------------------] 11.5% ...Iterating
        [=======-----------------------------------------------------] 11.6% ...Iterating
        [=======-----------------------------------------------------] 11.7% ...Iterating
        [=======-----------------------------------------------------] 11.8% ...Iterating
        [=======-----------------------------------------------------] 11.9% ...Iterating
        [=======-----------------------------------------------------] 12.0% ...Iterating
        [=======-----------------------------------------------------] 12.1% ...Iterating
        [=======-----------------------------------------------------] 12.2% ...Iterating
        [=======-----------------------------------------------------] 12.3% ...Iterating
        [=======-----------------------------------------------------] 12.4% ...Iterating
        [=======-----------------------------------------------------] 12.5% ...Iterating
        [========----------------------------------------------------] 12.6% ...Iterating
        [========----------------------------------------------------] 12.7% ...Iterating
        [========----------------------------------------------------] 12.8% ...Iterating
        [========----------------------------------------------------] 12.9% ...Iterating
        [========----------------------------------------------------] 13.0% ...Iterating
        [========----------------------------------------------------] 13.1% ...Iterating
        [========----------------------------------------------------] 13.2% ...Iterating
        [========----------------------------------------------------] 13.3% ...Iterating
        [========----------------------------------------------------] 13.4% ...Iterating
        [========----------------------------------------------------] 13.5% ...Iterating
        [========----------------------------------------------------] 13.6% ...Iterating
        [========----------------------------------------------------] 13.7% ...Iterating
        [========----------------------------------------------------] 13.8% ...Iterating
        [========----------------------------------------------------] 13.9% ...Iterating
        [========----------------------------------------------------] 14.0% ...Iterating
        [========----------------------------------------------------] 14.1% ...Iterating
        [=========---------------------------------------------------] 14.2% ...Iterating
        [=========---------------------------------------------------] 14.3% ...Iterating
        [=========---------------------------------------------------] 14.4% ...Iterating
        [=========---------------------------------------------------] 14.5% ...Iterating
        [=========---------------------------------------------------] 14.6% ...Iterating
        [=========---------------------------------------------------] 14.7% ...Iterating
        [=========---------------------------------------------------] 14.8% ...Iterating
        [=========---------------------------------------------------] 14.9% ...Iterating
        [=========---------------------------------------------------] 15.0% ...Iterating
        [=========---------------------------------------------------] 15.1% ...Iterating
        [=========---------------------------------------------------] 15.2% ...Iterating
        [=========---------------------------------------------------] 15.3% ...Iterating
        [=========---------------------------------------------------] 15.4% ...Iterating
        [=========---------------------------------------------------] 15.5% ...Iterating
        [=========---------------------------------------------------] 15.6% ...Iterating
        [=========---------------------------------------------------] 15.7% ...Iterating
        [=========---------------------------------------------------] 15.8% ...Iterating
        [==========--------------------------------------------------] 15.9% ...Iterating
        [==========--------------------------------------------------] 16.0% ...Iterating
        [==========--------------------------------------------------] 16.1% ...Iterating
        [==========--------------------------------------------------] 16.2% ...Iterating
        [==========--------------------------------------------------] 16.3% ...Iterating
        [==========--------------------------------------------------] 16.4% ...Iterating
        [==========--------------------------------------------------] 16.5% ...Iterating
        [==========--------------------------------------------------] 16.6% ...Iterating
        [==========--------------------------------------------------] 16.7% ...Iterating
        [==========--------------------------------------------------] 16.8% ...Iterating
        [==========--------------------------------------------------] 16.9% ...Iterating
        [==========--------------------------------------------------] 17.0% ...Iterating
        [==========--------------------------------------------------] 17.1% ...Iterating
        [==========--------------------------------------------------] 17.2% ...Iterating
        [==========--------------------------------------------------] 17.3% ...Iterating
        [==========--------------------------------------------------] 17.4% ...Iterating
        [==========--------------------------------------------------] 17.5% ...Iterating
        [===========-------------------------------------------------] 17.6% ...Iterating
        [===========-------------------------------------------------] 17.7% ...Iterating
        [===========-------------------------------------------------] 17.8% ...Iterating
        [===========-------------------------------------------------] 17.9% ...Iterating
        [===========-------------------------------------------------] 18.0% ...Iterating
        [===========-------------------------------------------------] 18.1% ...Iterating
        [===========-------------------------------------------------] 18.2% ...Iterating
        [===========-------------------------------------------------] 18.3% ...Iterating
        [===========-------------------------------------------------] 18.4% ...Iterating
        [===========-------------------------------------------------] 18.5% ...Iterating
        [===========-------------------------------------------------] 18.6% ...Iterating
        [===========-------------------------------------------------] 18.7% ...Iterating
        [===========-------------------------------------------------] 18.8% ...Iterating
        [===========-------------------------------------------------] 18.9% ...Iterating
        [===========-------------------------------------------------] 19.0% ...Iterating
        [===========-------------------------------------------------] 19.1% ...Iterating
        [============------------------------------------------------] 19.2% ...Iterating
        [============------------------------------------------------] 19.3% ...Iterating
        [============------------------------------------------------] 19.4% ...Iterating
        [============------------------------------------------------] 19.5% ...Iterating
        [============------------------------------------------------] 19.6% ...Iterating
        [============------------------------------------------------] 19.7% ...Iterating
        [============------------------------------------------------] 19.8% ...Iterating
        [============------------------------------------------------] 19.9% ...Iterating
        [============------------------------------------------------] 20.0% ...Iterating
        [============------------------------------------------------] 20.1% ...Iterating
        [============------------------------------------------------] 20.2% ...Iterating
        [============------------------------------------------------] 20.3% ...Iterating
        [============------------------------------------------------] 20.4% ...Iterating
        [============------------------------------------------------] 20.5% ...Iterating
        [============------------------------------------------------] 20.6% ...Iterating
        [============------------------------------------------------] 20.7% ...Iterating
        [============------------------------------------------------] 20.8% ...Iterating
        [=============-----------------------------------------------] 20.9% ...Iterating
        [=============-----------------------------------------------] 21.0% ...Iterating
        [=============-----------------------------------------------] 21.1% ...Iterating
        [=============-----------------------------------------------] 21.2% ...Iterating
        [=============-----------------------------------------------] 21.3% ...Iterating
        [=============-----------------------------------------------] 21.4% ...Iterating
        [=============-----------------------------------------------] 21.5% ...Iterating
        [=============-----------------------------------------------] 21.6% ...Iterating
        [=============-----------------------------------------------] 21.7% ...Iterating
        [=============-----------------------------------------------] 21.8% ...Iterating
        [=============-----------------------------------------------] 21.9% ...Iterating
        [=============-----------------------------------------------] 22.0% ...Iterating
        [=============-----------------------------------------------] 22.1% ...Iterating
        [=============-----------------------------------------------] 22.2% ...Iterating
        [=============-----------------------------------------------] 22.3% ...Iterating
        [=============-----------------------------------------------] 22.4% ...Iterating
        [=============-----------------------------------------------] 22.5% ...Iterating
        [==============----------------------------------------------] 22.6% ...Iterating
        [==============----------------------------------------------] 22.7% ...Iterating
        [==============----------------------------------------------] 22.8% ...Iterating
        [==============----------------------------------------------] 22.9% ...Iterating
        [==============----------------------------------------------] 23.0% ...Iterating
        [==============----------------------------------------------] 23.1% ...Iterating
        [==============----------------------------------------------] 23.2% ...Iterating
        [==============----------------------------------------------] 23.3% ...Iterating
        [==============----------------------------------------------] 23.4% ...Iterating
        [==============----------------------------------------------] 23.5% ...Iterating
        [==============----------------------------------------------] 23.6% ...Iterating
        [==============----------------------------------------------] 23.7% ...Iterating
        [==============----------------------------------------------] 23.8% ...Iterating
        [==============----------------------------------------------] 23.9% ...Iterating
        [==============----------------------------------------------] 24.0% ...Iterating
        [==============----------------------------------------------] 24.1% ...Iterating
        [===============---------------------------------------------] 24.2% ...Iterating
        [===============---------------------------------------------] 24.3% ...Iterating
        [===============---------------------------------------------] 24.4% ...Iterating
        [===============---------------------------------------------] 24.5% ...Iterating
        [===============---------------------------------------------] 24.6% ...Iterating
        [===============---------------------------------------------] 24.7% ...Iterating
        [===============---------------------------------------------] 24.8% ...Iterating
        [===============---------------------------------------------] 24.9% ...Iterating
        [===============---------------------------------------------] 25.0% ...Iterating
        [===============---------------------------------------------] 25.1% ...Iterating
        [===============---------------------------------------------] 25.2% ...Iterating
        [===============---------------------------------------------] 25.3% ...Iterating
        [===============---------------------------------------------] 25.4% ...Iterating
        [===============---------------------------------------------] 25.5% ...Iterating
        [===============---------------------------------------------] 25.6% ...Iterating
        [===============---------------------------------------------] 25.7% ...Iterating
        [===============---------------------------------------------] 25.8% ...Iterating
        [================--------------------------------------------] 25.9% ...Iterating
        [================--------------------------------------------] 26.0% ...Iterating
        [================--------------------------------------------] 26.1% ...Iterating
        [================--------------------------------------------] 26.2% ...Iterating
        [================--------------------------------------------] 26.3% ...Iterating
        [================--------------------------------------------] 26.4% ...Iterating
        [================--------------------------------------------] 26.5% ...Iterating
        [================--------------------------------------------] 26.6% ...Iterating
        [================--------------------------------------------] 26.7% ...Iterating
        [================--------------------------------------------] 26.8% ...Iterating
        [================--------------------------------------------] 26.9% ...Iterating
        [================--------------------------------------------] 27.0% ...Iterating
        [================--------------------------------------------] 27.1% ...Iterating
        [================--------------------------------------------] 27.2% ...Iterating
        [================--------------------------------------------] 27.3% ...Iterating
        [================--------------------------------------------] 27.4% ...Iterating
        [================--------------------------------------------] 27.5% ...Iterating
        [=================-------------------------------------------] 27.6% ...Iterating
        [=================-------------------------------------------] 27.7% ...Iterating
        [=================-------------------------------------------] 27.8% ...Iterating
        [=================-------------------------------------------] 27.9% ...Iterating
        [=================-------------------------------------------] 28.0% ...Iterating
        [=================-------------------------------------------] 28.1% ...Iterating
        [=================-------------------------------------------] 28.2% ...Iterating
        [=================-------------------------------------------] 28.3% ...Iterating
        [=================-------------------------------------------] 28.4% ...Iterating
        [=================-------------------------------------------] 28.5% ...Iterating
        [=================-------------------------------------------] 28.6% ...Iterating
        [=================-------------------------------------------] 28.7% ...Iterating
        [=================-------------------------------------------] 28.8% ...Iterating
        [=================-------------------------------------------] 28.9% ...Iterating
        [=================-------------------------------------------] 29.0% ...Iterating
        [=================-------------------------------------------] 29.1% ...Iterating
        [==================------------------------------------------] 29.2% ...Iterating
        [==================------------------------------------------] 29.3% ...Iterating
        [==================------------------------------------------] 29.4% ...Iterating
        [==================------------------------------------------] 29.5% ...Iterating
        [==================------------------------------------------] 29.6% ...Iterating
        [==================------------------------------------------] 29.7% ...Iterating
        [==================------------------------------------------] 29.8% ...Iterating
        [==================------------------------------------------] 29.9% ...Iterating
        [==================------------------------------------------] 30.0% ...Iterating
        [==================------------------------------------------] 30.1% ...Iterating
        [==================------------------------------------------] 30.2% ...Iterating
        [==================------------------------------------------] 30.3% ...Iterating
        [==================------------------------------------------] 30.4% ...Iterating
        [==================------------------------------------------] 30.5% ...Iterating
        [==================------------------------------------------] 30.6% ...Iterating
        [==================------------------------------------------] 30.7% ...Iterating
        [==================------------------------------------------] 30.8% ...Iterating
        [===================-----------------------------------------] 30.9% ...Iterating
        [===================-----------------------------------------] 31.0% ...Iterating
        [===================-----------------------------------------] 31.1% ...Iterating
        [===================-----------------------------------------] 31.2% ...Iterating
        [===================-----------------------------------------] 31.3% ...Iterating
        [===================-----------------------------------------] 31.4% ...Iterating
        [===================-----------------------------------------] 31.5% ...Iterating
        [===================-----------------------------------------] 31.6% ...Iterating
        [===================-----------------------------------------] 31.7% ...Iterating
        [===================-----------------------------------------] 31.8% ...Iterating
        [===================-----------------------------------------] 31.9% ...Iterating
        [===================-----------------------------------------] 32.0% ...Iterating
        [===================-----------------------------------------] 32.1% ...Iterating
        [===================-----------------------------------------] 32.2% ...Iterating
        [===================-----------------------------------------] 32.3% ...Iterating
        [===================-----------------------------------------] 32.4% ...Iterating
        [===================-----------------------------------------] 32.5% ...Iterating
        [====================----------------------------------------] 32.6% ...Iterating
        [====================----------------------------------------] 32.7% ...Iterating
        [====================----------------------------------------] 32.8% ...Iterating
        [====================----------------------------------------] 32.9% ...Iterating
        [====================----------------------------------------] 33.0% ...Iterating
        [====================----------------------------------------] 33.1% ...Iterating
        [====================----------------------------------------] 33.2% ...Iterating
        [====================----------------------------------------] 33.3% ...Iterating
        [====================----------------------------------------] 33.4% ...Iterating
        [====================----------------------------------------] 33.5% ...Iterating
        [====================----------------------------------------] 33.6% ...Iterating
        [====================----------------------------------------] 33.7% ...Iterating
        [====================----------------------------------------] 33.8% ...Iterating
        [====================----------------------------------------] 33.9% ...Iterating
        [====================----------------------------------------] 34.0% ...Iterating
        [====================----------------------------------------] 34.1% ...Iterating
        [====================----------------------------------------] 34.2% ...Iterating
        [=====================---------------------------------------] 34.3% ...Iterating
        [=====================---------------------------------------] 34.4% ...Iterating
        [=====================---------------------------------------] 34.5% ...Iterating
        [=====================---------------------------------------] 34.6% ...Iterating
        [=====================---------------------------------------] 34.7% ...Iterating
        [=====================---------------------------------------] 34.8% ...Iterating
        [=====================---------------------------------------] 34.9% ...Iterating
        [=====================---------------------------------------] 35.0% ...Iterating
        [=====================---------------------------------------] 35.1% ...Iterating
        [=====================---------------------------------------] 35.2% ...Iterating
        [=====================---------------------------------------] 35.3% ...Iterating
        [=====================---------------------------------------] 35.4% ...Iterating
        [=====================---------------------------------------] 35.5% ...Iterating
        [=====================---------------------------------------] 35.6% ...Iterating
        [=====================---------------------------------------] 35.7% ...Iterating
        [=====================---------------------------------------] 35.8% ...Iterating
        [======================--------------------------------------] 35.9% ...Iterating
        [======================--------------------------------------] 36.0% ...Iterating
        [======================--------------------------------------] 36.1% ...Iterating
        [======================--------------------------------------] 36.2% ...Iterating
        [======================--------------------------------------] 36.3% ...Iterating
        [======================--------------------------------------] 36.4% ...Iterating
        [======================--------------------------------------] 36.5% ...Iterating
        [======================--------------------------------------] 36.6% ...Iterating
        [======================--------------------------------------] 36.7% ...Iterating
        [======================--------------------------------------] 36.8% ...Iterating
        [======================--------------------------------------] 36.9% ...Iterating
        [======================--------------------------------------] 37.0% ...Iterating
        [======================--------------------------------------] 37.1% ...Iterating
        [======================--------------------------------------] 37.2% ...Iterating
        [======================--------------------------------------] 37.3% ...Iterating
        [======================--------------------------------------] 37.4% ...Iterating
        [======================--------------------------------------] 37.5% ...Iterating
        [=======================-------------------------------------] 37.6% ...Iterating
        [=======================-------------------------------------] 37.7% ...Iterating
        [=======================-------------------------------------] 37.8% ...Iterating
        [=======================-------------------------------------] 37.9% ...Iterating
        [=======================-------------------------------------] 38.0% ...Iterating
        [=======================-------------------------------------] 38.1% ...Iterating
        [=======================-------------------------------------] 38.2% ...Iterating
        [=======================-------------------------------------] 38.3% ...Iterating
        [=======================-------------------------------------] 38.4% ...Iterating
        [=======================-------------------------------------] 38.5% ...Iterating
        [=======================-------------------------------------] 38.6% ...Iterating
        [=======================-------------------------------------] 38.7% ...Iterating
        [=======================-------------------------------------] 38.8% ...Iterating
        [=======================-------------------------------------] 38.9% ...Iterating
        [=======================-------------------------------------] 39.0% ...Iterating
        [=======================-------------------------------------] 39.1% ...Iterating
        [=======================-------------------------------------] 39.2% ...Iterating
        [========================------------------------------------] 39.3% ...Iterating
        [========================------------------------------------] 39.4% ...Iterating
        [========================------------------------------------] 39.5% ...Iterating
        [========================------------------------------------] 39.6% ...Iterating
        [========================------------------------------------] 39.7% ...Iterating
        [========================------------------------------------] 39.8% ...Iterating
        [========================------------------------------------] 39.9% ...Iterating
        [========================------------------------------------] 40.0% ...Iterating
        [========================------------------------------------] 40.1% ...Iterating
        [========================------------------------------------] 40.2% ...Iterating
        [========================------------------------------------] 40.3% ...Iterating
        [========================------------------------------------] 40.4% ...Iterating
        [========================------------------------------------] 40.5% ...Iterating
        [========================------------------------------------] 40.6% ...Iterating
        [========================------------------------------------] 40.7% ...Iterating
        [========================------------------------------------] 40.8% ...Iterating
        [=========================-----------------------------------] 40.9% ...Iterating
        [=========================-----------------------------------] 41.0% ...Iterating
        [=========================-----------------------------------] 41.1% ...Iterating
        [=========================-----------------------------------] 41.2% ...Iterating
        [=========================-----------------------------------] 41.3% ...Iterating
        [=========================-----------------------------------] 41.4% ...Iterating
        [=========================-----------------------------------] 41.5% ...Iterating
        [=========================-----------------------------------] 41.6% ...Iterating
        [=========================-----------------------------------] 41.7% ...Iterating
        [=========================-----------------------------------] 41.8% ...Iterating
        [=========================-----------------------------------] 41.9% ...Iterating
        [=========================-----------------------------------] 42.0% ...Iterating
        [=========================-----------------------------------] 42.1% ...Iterating
        [=========================-----------------------------------] 42.2% ...Iterating
        [=========================-----------------------------------] 42.3% ...Iterating
        [=========================-----------------------------------] 42.4% ...Iterating
        [=========================-----------------------------------] 42.5% ...Iterating
        [==========================----------------------------------] 42.6% ...Iterating
        [==========================----------------------------------] 42.7% ...Iterating
        [==========================----------------------------------] 42.8% ...Iterating
        [==========================----------------------------------] 42.9% ...Iterating
        [==========================----------------------------------] 43.0% ...Iterating
        [==========================----------------------------------] 43.1% ...Iterating
        [==========================----------------------------------] 43.2% ...Iterating
        [==========================----------------------------------] 43.3% ...Iterating
        [==========================----------------------------------] 43.4% ...Iterating
        [==========================----------------------------------] 43.5% ...Iterating
        [==========================----------------------------------] 43.6% ...Iterating
        [==========================----------------------------------] 43.7% ...Iterating
        [==========================----------------------------------] 43.8% ...Iterating
        [==========================----------------------------------] 43.9% ...Iterating
        [==========================----------------------------------] 44.0% ...Iterating
        [==========================----------------------------------] 44.1% ...Iterating
        [==========================----------------------------------] 44.2% ...Iterating
        [===========================---------------------------------] 44.3% ...Iterating
        [===========================---------------------------------] 44.4% ...Iterating
        [===========================---------------------------------] 44.5% ...Iterating
        [===========================---------------------------------] 44.6% ...Iterating
        [===========================---------------------------------] 44.7% ...Iterating
        [===========================---------------------------------] 44.8% ...Iterating
        [===========================---------------------------------] 44.9% ...Iterating
        [===========================---------------------------------] 45.0% ...Iterating
        [===========================---------------------------------] 45.1% ...Iterating
        [===========================---------------------------------] 45.2% ...Iterating
        [===========================---------------------------------] 45.3% ...Iterating
        [===========================---------------------------------] 45.4% ...Iterating
        [===========================---------------------------------] 45.5% ...Iterating
        [===========================---------------------------------] 45.6% ...Iterating
        [===========================---------------------------------] 45.7% ...Iterating
        [===========================---------------------------------] 45.8% ...Iterating
        [============================--------------------------------] 45.9% ...Iterating
        [============================--------------------------------] 46.0% ...Iterating
        [============================--------------------------------] 46.1% ...Iterating
        [============================--------------------------------] 46.2% ...Iterating
        [============================--------------------------------] 46.3% ...Iterating
        [============================--------------------------------] 46.4% ...Iterating
        [============================--------------------------------] 46.5% ...Iterating
        [============================--------------------------------] 46.6% ...Iterating
        [============================--------------------------------] 46.7% ...Iterating
        [============================--------------------------------] 46.8% ...Iterating
        [============================--------------------------------] 46.9% ...Iterating
        [============================--------------------------------] 47.0% ...Iterating
        [============================--------------------------------] 47.1% ...Iterating
        [============================--------------------------------] 47.2% ...Iterating
        [============================--------------------------------] 47.3% ...Iterating
        [============================--------------------------------] 47.4% ...Iterating
        [============================--------------------------------] 47.5% ...Iterating
        [=============================-------------------------------] 47.6% ...Iterating
        [=============================-------------------------------] 47.7% ...Iterating
        [=============================-------------------------------] 47.8% ...Iterating
        [=============================-------------------------------] 47.9% ...Iterating
        [=============================-------------------------------] 48.0% ...Iterating
        [=============================-------------------------------] 48.1% ...Iterating
        [=============================-------------------------------] 48.2% ...Iterating
        [=============================-------------------------------] 48.3% ...Iterating
        [=============================-------------------------------] 48.4% ...Iterating
        [=============================-------------------------------] 48.5% ...Iterating
        [=============================-------------------------------] 48.6% ...Iterating
        [=============================-------------------------------] 48.7% ...Iterating
        [=============================-------------------------------] 48.8% ...Iterating
        [=============================-------------------------------] 48.9% ...Iterating
        [=============================-------------------------------] 49.0% ...Iterating
        [=============================-------------------------------] 49.1% ...Iterating
        [=============================-------------------------------] 49.2% ...Iterating
        [==============================------------------------------] 49.3% ...Iterating
        [==============================------------------------------] 49.4% ...Iterating
        [==============================------------------------------] 49.5% ...Iterating
        [==============================------------------------------] 49.6% ...Iterating
        [==============================------------------------------] 49.7% ...Iterating
        [==============================------------------------------] 49.8% ...Iterating
        [==============================------------------------------] 49.9% ...Iterating
        [==============================------------------------------] 50.0% ...Iterating
        [==============================------------------------------] 50.0% ...Iterating
        [==============================------------------------------] 50.1% ...Iterating
        [==============================------------------------------] 50.2% ...Iterating
        [==============================------------------------------] 50.3% ...Iterating
        [==============================------------------------------] 50.4% ...Iterating
        [==============================------------------------------] 50.5% ...Iterating
        [==============================------------------------------] 50.6% ...Iterating
        [==============================------------------------------] 50.7% ...Iterating
        [===============================-----------------------------] 50.8% ...Iterating
        [===============================-----------------------------] 50.9% ...Iterating
        [===============================-----------------------------] 51.0% ...Iterating
        [===============================-----------------------------] 51.1% ...Iterating
        [===============================-----------------------------] 51.2% ...Iterating
        [===============================-----------------------------] 51.3% ...Iterating
        [===============================-----------------------------] 51.4% ...Iterating
        [===============================-----------------------------] 51.5% ...Iterating
        [===============================-----------------------------] 51.6% ...Iterating
        [===============================-----------------------------] 51.7% ...Iterating
        [===============================-----------------------------] 51.8% ...Iterating
        [===============================-----------------------------] 51.9% ...Iterating
        [===============================-----------------------------] 52.0% ...Iterating
        [===============================-----------------------------] 52.1% ...Iterating
        [===============================-----------------------------] 52.2% ...Iterating
        [===============================-----------------------------] 52.3% ...Iterating
        [===============================-----------------------------] 52.4% ...Iterating
        [================================----------------------------] 52.5% ...Iterating
        [================================----------------------------] 52.6% ...Iterating
        [================================----------------------------] 52.7% ...Iterating
        [================================----------------------------] 52.8% ...Iterating
        [================================----------------------------] 52.9% ...Iterating
        [================================----------------------------] 53.0% ...Iterating
        [================================----------------------------] 53.1% ...Iterating
        [================================----------------------------] 53.2% ...Iterating
        [================================----------------------------] 53.3% ...Iterating
        [================================----------------------------] 53.4% ...Iterating
        [================================----------------------------] 53.5% ...Iterating
        [================================----------------------------] 53.6% ...Iterating
        [================================----------------------------] 53.7% ...Iterating
        [================================----------------------------] 53.8% ...Iterating
        [================================----------------------------] 53.9% ...Iterating
        [================================----------------------------] 54.0% ...Iterating
        [================================----------------------------] 54.1% ...Iterating
        [=================================---------------------------] 54.2% ...Iterating
        [=================================---------------------------] 54.3% ...Iterating
        [=================================---------------------------] 54.4% ...Iterating
        [=================================---------------------------] 54.5% ...Iterating
        [=================================---------------------------] 54.6% ...Iterating
        [=================================---------------------------] 54.7% ...Iterating
        [=================================---------------------------] 54.8% ...Iterating
        [=================================---------------------------] 54.9% ...Iterating
        [=================================---------------------------] 55.0% ...Iterating
        [=================================---------------------------] 55.1% ...Iterating
        [=================================---------------------------] 55.2% ...Iterating
        [=================================---------------------------] 55.3% ...Iterating
        [=================================---------------------------] 55.4% ...Iterating
        [=================================---------------------------] 55.5% ...Iterating
        [=================================---------------------------] 55.6% ...Iterating
        [=================================---------------------------] 55.7% ...Iterating
        [==================================--------------------------] 55.8% ...Iterating
        [==================================--------------------------] 55.9% ...Iterating
        [==================================--------------------------] 56.0% ...Iterating
        [==================================--------------------------] 56.1% ...Iterating
        [==================================--------------------------] 56.2% ...Iterating
        [==================================--------------------------] 56.3% ...Iterating
        [==================================--------------------------] 56.4% ...Iterating
        [==================================--------------------------] 56.5% ...Iterating
        [==================================--------------------------] 56.6% ...Iterating
        [==================================--------------------------] 56.7% ...Iterating
        [==================================--------------------------] 56.8% ...Iterating
        [==================================--------------------------] 56.9% ...Iterating
        [==================================--------------------------] 57.0% ...Iterating
        [==================================--------------------------] 57.1% ...Iterating
        [==================================--------------------------] 57.2% ...Iterating
        [==================================--------------------------] 57.3% ...Iterating
        [==================================--------------------------] 57.4% ...Iterating
        [===================================-------------------------] 57.5% ...Iterating
        [===================================-------------------------] 57.6% ...Iterating
        [===================================-------------------------] 57.7% ...Iterating
        [===================================-------------------------] 57.8% ...Iterating
        [===================================-------------------------] 57.9% ...Iterating
        [===================================-------------------------] 58.0% ...Iterating
        [===================================-------------------------] 58.1% ...Iterating
        [===================================-------------------------] 58.2% ...Iterating
        [===================================-------------------------] 58.3% ...Iterating
        [===================================-------------------------] 58.4% ...Iterating
        [===================================-------------------------] 58.5% ...Iterating
        [===================================-------------------------] 58.6% ...Iterating
        [===================================-------------------------] 58.7% ...Iterating
        [===================================-------------------------] 58.8% ...Iterating
        [===================================-------------------------] 58.9% ...Iterating
        [===================================-------------------------] 59.0% ...Iterating
        [===================================-------------------------] 59.1% ...Iterating
        [====================================------------------------] 59.2% ...Iterating
        [====================================------------------------] 59.3% ...Iterating
        [====================================------------------------] 59.4% ...Iterating
        [====================================------------------------] 59.5% ...Iterating
        [====================================------------------------] 59.6% ...Iterating
        [====================================------------------------] 59.7% ...Iterating
        [====================================------------------------] 59.8% ...Iterating
        [====================================------------------------] 59.9% ...Iterating
        [====================================------------------------] 60.0% ...Iterating
        [====================================------------------------] 60.1% ...Iterating
        [====================================------------------------] 60.2% ...Iterating
        [====================================------------------------] 60.3% ...Iterating
        [====================================------------------------] 60.4% ...Iterating
        [====================================------------------------] 60.5% ...Iterating
        [====================================------------------------] 60.6% ...Iterating
        [====================================------------------------] 60.7% ...Iterating
        [=====================================-----------------------] 60.8% ...Iterating
        [=====================================-----------------------] 60.9% ...Iterating
        [=====================================-----------------------] 61.0% ...Iterating
        [=====================================-----------------------] 61.1% ...Iterating
        [=====================================-----------------------] 61.2% ...Iterating
        [=====================================-----------------------] 61.3% ...Iterating
        [=====================================-----------------------] 61.4% ...Iterating
        [=====================================-----------------------] 61.5% ...Iterating
        [=====================================-----------------------] 61.6% ...Iterating
        [=====================================-----------------------] 61.7% ...Iterating
        [=====================================-----------------------] 61.8% ...Iterating
        [=====================================-----------------------] 61.9% ...Iterating
        [=====================================-----------------------] 62.0% ...Iterating
        [=====================================-----------------------] 62.1% ...Iterating
        [=====================================-----------------------] 62.2% ...Iterating
        [=====================================-----------------------] 62.3% ...Iterating
        [=====================================-----------------------] 62.4% ...Iterating
        [======================================----------------------] 62.5% ...Iterating
        [======================================----------------------] 62.6% ...Iterating
        [======================================----------------------] 62.7% ...Iterating
        [======================================----------------------] 62.8% ...Iterating
        [======================================----------------------] 62.9% ...Iterating
        [======================================----------------------] 63.0% ...Iterating
        [======================================----------------------] 63.1% ...Iterating
        [======================================----------------------] 63.2% ...Iterating
        [======================================----------------------] 63.3% ...Iterating
        [======================================----------------------] 63.4% ...Iterating
        [======================================----------------------] 63.5% ...Iterating
        [======================================----------------------] 63.6% ...Iterating
        [======================================----------------------] 63.7% ...Iterating
        [======================================----------------------] 63.8% ...Iterating
        [======================================----------------------] 63.9% ...Iterating
        [======================================----------------------] 64.0% ...Iterating
        [======================================----------------------] 64.1% ...Iterating
        [=======================================---------------------] 64.2% ...Iterating
        [=======================================---------------------] 64.3% ...Iterating
        [=======================================---------------------] 64.4% ...Iterating
        [=======================================---------------------] 64.5% ...Iterating
        [=======================================---------------------] 64.6% ...Iterating
        [=======================================---------------------] 64.7% ...Iterating
        [=======================================---------------------] 64.8% ...Iterating
        [=======================================---------------------] 64.9% ...Iterating
        [=======================================---------------------] 65.0% ...Iterating
        [=======================================---------------------] 65.1% ...Iterating
        [=======================================---------------------] 65.2% ...Iterating
        [=======================================---------------------] 65.3% ...Iterating
        [=======================================---------------------] 65.4% ...Iterating
        [=======================================---------------------] 65.5% ...Iterating
        [=======================================---------------------] 65.6% ...Iterating
        [=======================================---------------------] 65.7% ...Iterating
        [========================================--------------------] 65.8% ...Iterating
        [========================================--------------------] 65.9% ...Iterating
        [========================================--------------------] 66.0% ...Iterating
        [========================================--------------------] 66.1% ...Iterating
        [========================================--------------------] 66.2% ...Iterating
        [========================================--------------------] 66.3% ...Iterating
        [========================================--------------------] 66.4% ...Iterating
        [========================================--------------------] 66.5% ...Iterating
        [========================================--------------------] 66.6% ...Iterating
        [========================================--------------------] 66.7% ...Iterating
        [========================================--------------------] 66.8% ...Iterating
        [========================================--------------------] 66.9% ...Iterating
        [========================================--------------------] 67.0% ...Iterating
        [========================================--------------------] 67.1% ...Iterating
        [========================================--------------------] 67.2% ...Iterating
        [========================================--------------------] 67.3% ...Iterating
        [========================================--------------------] 67.4% ...Iterating
        [=========================================-------------------] 67.5% ...Iterating
        [=========================================-------------------] 67.6% ...Iterating
        [=========================================-------------------] 67.7% ...Iterating
        [=========================================-------------------] 67.8% ...Iterating
        [=========================================-------------------] 67.9% ...Iterating
        [=========================================-------------------] 68.0% ...Iterating
        [=========================================-------------------] 68.1% ...Iterating
        [=========================================-------------------] 68.2% ...Iterating
        [=========================================-------------------] 68.3% ...Iterating
        [=========================================-------------------] 68.4% ...Iterating
        [=========================================-------------------] 68.5% ...Iterating
        [=========================================-------------------] 68.6% ...Iterating
        [=========================================-------------------] 68.7% ...Iterating
        [=========================================-------------------] 68.8% ...Iterating
        [=========================================-------------------] 68.9% ...Iterating
        [=========================================-------------------] 69.0% ...Iterating
        [=========================================-------------------] 69.1% ...Iterating
        [==========================================------------------] 69.2% ...Iterating
        [==========================================------------------] 69.3% ...Iterating
        [==========================================------------------] 69.4% ...Iterating
        [==========================================------------------] 69.5% ...Iterating
        [==========================================------------------] 69.6% ...Iterating
        [==========================================------------------] 69.7% ...Iterating
        [==========================================------------------] 69.8% ...Iterating
        [==========================================------------------] 69.9% ...Iterating
        [==========================================------------------] 70.0% ...Iterating
        [==========================================------------------] 70.1% ...Iterating
        [==========================================------------------] 70.2% ...Iterating
        [==========================================------------------] 70.3% ...Iterating
        [==========================================------------------] 70.4% ...Iterating
        [==========================================------------------] 70.5% ...Iterating
        [==========================================------------------] 70.6% ...Iterating
        [==========================================------------------] 70.7% ...Iterating
        [==========================================------------------] 70.8% ...Iterating
        [===========================================-----------------] 70.9% ...Iterating
        [===========================================-----------------] 71.0% ...Iterating
        [===========================================-----------------] 71.1% ...Iterating
        [===========================================-----------------] 71.2% ...Iterating
        [===========================================-----------------] 71.3% ...Iterating
        [===========================================-----------------] 71.4% ...Iterating
        [===========================================-----------------] 71.5% ...Iterating
        [===========================================-----------------] 71.6% ...Iterating
        [===========================================-----------------] 71.7% ...Iterating
        [===========================================-----------------] 71.8% ...Iterating
        [===========================================-----------------] 71.9% ...Iterating
        [===========================================-----------------] 72.0% ...Iterating
        [===========================================-----------------] 72.1% ...Iterating
        [===========================================-----------------] 72.2% ...Iterating
        [===========================================-----------------] 72.3% ...Iterating
        [===========================================-----------------] 72.4% ...Iterating
        [============================================----------------] 72.5% ...Iterating
        [============================================----------------] 72.6% ...Iterating
        [============================================----------------] 72.7% ...Iterating
        [============================================----------------] 72.8% ...Iterating
        [============================================----------------] 72.9% ...Iterating
        [============================================----------------] 73.0% ...Iterating
        [============================================----------------] 73.1% ...Iterating
        [============================================----------------] 73.2% ...Iterating
        [============================================----------------] 73.3% ...Iterating
        [============================================----------------] 73.4% ...Iterating
        [============================================----------------] 73.5% ...Iterating
        [============================================----------------] 73.6% ...Iterating
        [============================================----------------] 73.7% ...Iterating
        [============================================----------------] 73.8% ...Iterating
        [============================================----------------] 73.9% ...Iterating
        [============================================----------------] 74.0% ...Iterating
        [============================================----------------] 74.1% ...Iterating
        [=============================================---------------] 74.2% ...Iterating
        [=============================================---------------] 74.3% ...Iterating
        [=============================================---------------] 74.4% ...Iterating
        [=============================================---------------] 74.5% ...Iterating
        [=============================================---------------] 74.6% ...Iterating
        [=============================================---------------] 74.7% ...Iterating
        [=============================================---------------] 74.8% ...Iterating
        [=============================================---------------] 74.9% ...Iterating
        [=============================================---------------] 75.0% ...Iterating
        [=============================================---------------] 75.1% ...Iterating
        [=============================================---------------] 75.2% ...Iterating
        [=============================================---------------] 75.3% ...Iterating
        [=============================================---------------] 75.4% ...Iterating
        [=============================================---------------] 75.5% ...Iterating
        [=============================================---------------] 75.6% ...Iterating
        [=============================================---------------] 75.7% ...Iterating
        [=============================================---------------] 75.8% ...Iterating
        [==============================================--------------] 75.9% ...Iterating
        [==============================================--------------] 76.0% ...Iterating
        [==============================================--------------] 76.1% ...Iterating
        [==============================================--------------] 76.2% ...Iterating
        [==============================================--------------] 76.3% ...Iterating
        [==============================================--------------] 76.4% ...Iterating
        [==============================================--------------] 76.5% ...Iterating
        [==============================================--------------] 76.6% ...Iterating
        [==============================================--------------] 76.7% ...Iterating
        [==============================================--------------] 76.8% ...Iterating
        [==============================================--------------] 76.9% ...Iterating
        [==============================================--------------] 77.0% ...Iterating
        [==============================================--------------] 77.1% ...Iterating
        [==============================================--------------] 77.2% ...Iterating
        [==============================================--------------] 77.3% ...Iterating
        [==============================================--------------] 77.4% ...Iterating
        [===============================================-------------] 77.5% ...Iterating
        [===============================================-------------] 77.6% ...Iterating
        [===============================================-------------] 77.7% ...Iterating
        [===============================================-------------] 77.8% ...Iterating
        [===============================================-------------] 77.9% ...Iterating
        [===============================================-------------] 78.0% ...Iterating
        [===============================================-------------] 78.1% ...Iterating
        [===============================================-------------] 78.2% ...Iterating
        [===============================================-------------] 78.3% ...Iterating
        [===============================================-------------] 78.4% ...Iterating
        [===============================================-------------] 78.5% ...Iterating
        [===============================================-------------] 78.6% ...Iterating
        [===============================================-------------] 78.7% ...Iterating
        [===============================================-------------] 78.8% ...Iterating
        [===============================================-------------] 78.9% ...Iterating
        [===============================================-------------] 79.0% ...Iterating
        [===============================================-------------] 79.1% ...Iterating
        [================================================------------] 79.2% ...Iterating
        [================================================------------] 79.3% ...Iterating
        [================================================------------] 79.4% ...Iterating
        [================================================------------] 79.5% ...Iterating
        [================================================------------] 79.6% ...Iterating
        [================================================------------] 79.7% ...Iterating
        [================================================------------] 79.8% ...Iterating
        [================================================------------] 79.9% ...Iterating
        [================================================------------] 80.0% ...Iterating
        [================================================------------] 80.1% ...Iterating
        [================================================------------] 80.2% ...Iterating
        [================================================------------] 80.3% ...Iterating
        [================================================------------] 80.4% ...Iterating
        [================================================------------] 80.5% ...Iterating
        [================================================------------] 80.6% ...Iterating
        [================================================------------] 80.7% ...Iterating
        [================================================------------] 80.8% ...Iterating
        [=================================================-----------] 80.9% ...Iterating
        [=================================================-----------] 81.0% ...Iterating
        [=================================================-----------] 81.1% ...Iterating
        [=================================================-----------] 81.2% ...Iterating
        [=================================================-----------] 81.3% ...Iterating
        [=================================================-----------] 81.4% ...Iterating
        [=================================================-----------] 81.5% ...Iterating
        [=================================================-----------] 81.6% ...Iterating
        [=================================================-----------] 81.7% ...Iterating
        [=================================================-----------] 81.8% ...Iterating
        [=================================================-----------] 81.9% ...Iterating
        [=================================================-----------] 82.0% ...Iterating
        [=================================================-----------] 82.1% ...Iterating
        [=================================================-----------] 82.2% ...Iterating
        [=================================================-----------] 82.3% ...Iterating
        [=================================================-----------] 82.4% ...Iterating
        [==================================================----------] 82.5% ...Iterating
        [==================================================----------] 82.6% ...Iterating
        [==================================================----------] 82.7% ...Iterating
        [==================================================----------] 82.8% ...Iterating
        [==================================================----------] 82.9% ...Iterating
        [==================================================----------] 83.0% ...Iterating
        [==================================================----------] 83.1% ...Iterating
        [==================================================----------] 83.2% ...Iterating
        [==================================================----------] 83.3% ...Iterating
        [==================================================----------] 83.4% ...Iterating
        [==================================================----------] 83.5% ...Iterating
        [==================================================----------] 83.6% ...Iterating
        [==================================================----------] 83.7% ...Iterating
        [==================================================----------] 83.8% ...Iterating
        [==================================================----------] 83.9% ...Iterating
        [==================================================----------] 84.0% ...Iterating
        [==================================================----------] 84.1% ...Iterating
        [===================================================---------] 84.2% ...Iterating
        [===================================================---------] 84.3% ...Iterating
        [===================================================---------] 84.4% ...Iterating
        [===================================================---------] 84.5% ...Iterating
        [===================================================---------] 84.6% ...Iterating
        [===================================================---------] 84.7% ...Iterating
        [===================================================---------] 84.8% ...Iterating
        [===================================================---------] 84.9% ...Iterating
        [===================================================---------] 85.0% ...Iterating
        [===================================================---------] 85.1% ...Iterating
        [===================================================---------] 85.2% ...Iterating
        [===================================================---------] 85.3% ...Iterating
        [===================================================---------] 85.4% ...Iterating
        [===================================================---------] 85.5% ...Iterating
        [===================================================---------] 85.6% ...Iterating
        [===================================================---------] 85.7% ...Iterating
        [===================================================---------] 85.8% ...Iterating
        [====================================================--------] 85.9% ...Iterating
        [====================================================--------] 86.0% ...Iterating
        [====================================================--------] 86.1% ...Iterating
        [====================================================--------] 86.2% ...Iterating
        [====================================================--------] 86.3% ...Iterating
        [====================================================--------] 86.4% ...Iterating
        [====================================================--------] 86.5% ...Iterating
        [====================================================--------] 86.6% ...Iterating
        [====================================================--------] 86.7% ...Iterating
        [====================================================--------] 86.8% ...Iterating
        [====================================================--------] 86.9% ...Iterating
        [====================================================--------] 87.0% ...Iterating
        [====================================================--------] 87.1% ...Iterating
        [====================================================--------] 87.2% ...Iterating
        [====================================================--------] 87.3% ...Iterating
        [====================================================--------] 87.4% ...Iterating
        [=====================================================-------] 87.5% ...Iterating
        [=====================================================-------] 87.6% ...Iterating
        [=====================================================-------] 87.7% ...Iterating
        [=====================================================-------] 87.8% ...Iterating
        [=====================================================-------] 87.9% ...Iterating
        [=====================================================-------] 88.0% ...Iterating
        [=====================================================-------] 88.1% ...Iterating
        [=====================================================-------] 88.2% ...Iterating
        [=====================================================-------] 88.3% ...Iterating
        [=====================================================-------] 88.4% ...Iterating
        [=====================================================-------] 88.5% ...Iterating
        [=====================================================-------] 88.6% ...Iterating
        [=====================================================-------] 88.7% ...Iterating
        [=====================================================-------] 88.8% ...Iterating
        [=====================================================-------] 88.9% ...Iterating
        [=====================================================-------] 89.0% ...Iterating
        [=====================================================-------] 89.1% ...Iterating
        [======================================================------] 89.2% ...Iterating
        [======================================================------] 89.3% ...Iterating
        [======================================================------] 89.4% ...Iterating
        [======================================================------] 89.5% ...Iterating
        [======================================================------] 89.6% ...Iterating
        [======================================================------] 89.7% ...Iterating
        [======================================================------] 89.8% ...Iterating
        [======================================================------] 89.9% ...Iterating
        [======================================================------] 90.0% ...Iterating
        [======================================================------] 90.1% ...Iterating
        [======================================================------] 90.2% ...Iterating
        [======================================================------] 90.3% ...Iterating
        [======================================================------] 90.4% ...Iterating
        [======================================================------] 90.5% ...Iterating
        [======================================================------] 90.6% ...Iterating
        [======================================================------] 90.7% ...Iterating
        [======================================================------] 90.8% ...Iterating
        [=======================================================-----] 90.9% ...Iterating
        [=======================================================-----] 91.0% ...Iterating
        [=======================================================-----] 91.1% ...Iterating
        [=======================================================-----] 91.2% ...Iterating
        [=======================================================-----] 91.3% ...Iterating
        [=======================================================-----] 91.4% ...Iterating
        [=======================================================-----] 91.5% ...Iterating
        [=======================================================-----] 91.6% ...Iterating
        [=======================================================-----] 91.7% ...Iterating
        [=======================================================-----] 91.8% ...Iterating
        [=======================================================-----] 91.9% ...Iterating
        [=======================================================-----] 92.0% ...Iterating
        [=======================================================-----] 92.1% ...Iterating
        [=======================================================-----] 92.2% ...Iterating
        [=======================================================-----] 92.3% ...Iterating
        [=======================================================-----] 92.4% ...Iterating
        [========================================================----] 92.5% ...Iterating
        [========================================================----] 92.6% ...Iterating
        [========================================================----] 92.7% ...Iterating
        [========================================================----] 92.8% ...Iterating
        [========================================================----] 92.9% ...Iterating
        [========================================================----] 93.0% ...Iterating
        [========================================================----] 93.1% ...Iterating
        [========================================================----] 93.2% ...Iterating
        [========================================================----] 93.3% ...Iterating
        [========================================================----] 93.4% ...Iterating
        [========================================================----] 93.5% ...Iterating
        [========================================================----] 93.6% ...Iterating
        [========================================================----] 93.7% ...Iterating
        [========================================================----] 93.8% ...Iterating
        [========================================================----] 93.9% ...Iterating
        [========================================================----] 94.0% ...Iterating
        [========================================================----] 94.1% ...Iterating
        [=========================================================---] 94.2% ...Iterating
        [=========================================================---] 94.3% ...Iterating
        [=========================================================---] 94.4% ...Iterating
        [=========================================================---] 94.5% ...Iterating
        [=========================================================---] 94.6% ...Iterating
        [=========================================================---] 94.7% ...Iterating
        [=========================================================---] 94.8% ...Iterating
        [=========================================================---] 94.9% ...Iterating
        [=========================================================---] 95.0% ...Iterating
        [=========================================================---] 95.1% ...Iterating
        [=========================================================---] 95.2% ...Iterating
        [=========================================================---] 95.3% ...Iterating
        [=========================================================---] 95.4% ...Iterating
        [=========================================================---] 95.5% ...Iterating
        [=========================================================---] 95.6% ...Iterating
        [=========================================================---] 95.7% ...Iterating
        [=========================================================---] 95.8% ...Iterating
        [==========================================================--] 95.9% ...Iterating
        [==========================================================--] 96.0% ...Iterating
        [==========================================================--] 96.1% ...Iterating
        [==========================================================--] 96.2% ...Iterating
        [==========================================================--] 96.3% ...Iterating
        [==========================================================--] 96.4% ...Iterating
        [==========================================================--] 96.5% ...Iterating
        [==========================================================--] 96.6% ...Iterating
        [==========================================================--] 96.7% ...Iterating
        [==========================================================--] 96.8% ...Iterating
        [==========================================================--] 96.9% ...Iterating
        [==========================================================--] 97.0% ...Iterating
        [==========================================================--] 97.1% ...Iterating
        [==========================================================--] 97.2% ...Iterating
        [==========================================================--] 97.3% ...Iterating
        [==========================================================--] 97.4% ...Iterating
        [===========================================================-] 97.5% ...Iterating
        [===========================================================-] 97.6% ...Iterating
        [===========================================================-] 97.7% ...Iterating
        [===========================================================-] 97.8% ...Iterating
        [===========================================================-] 97.9% ...Iterating
        [===========================================================-] 98.0% ...Iterating
        [===========================================================-] 98.1% ...Iterating
        [===========================================================-] 98.2% ...Iterating
        [===========================================================-] 98.3% ...Iterating
        [===========================================================-] 98.4% ...Iterating
        [===========================================================-] 98.5% ...Iterating
        [===========================================================-] 98.6% ...Iterating
        [===========================================================-] 98.7% ...Iterating
        [===========================================================-] 98.8% ...Iterating
        [===========================================================-] 98.9% ...Iterating
        [===========================================================-] 99.0% ...Iterating
        [===========================================================-] 99.1% ...Iterating
        [============================================================] 99.2% ...Iterating
        [============================================================] 99.3% ...Iterating
        [============================================================] 99.4% ...Iterating
        [============================================================] 99.5% ...Iterating
        [============================================================] 99.6% ...Iterating
        [============================================================] 99.7% ...Iterating
        [============================================================] 99.8% ...Iterating
        [============================================================] 99.9% ...Iterating
    """
    import sys

    bar_length = 60
    filled_len = int(round(bar_length * current_progress / float(maximum_progress)))

    percents = round(100.0 * current_progress / float(maximum_progress), 1)
    bar = '=' * filled_len + '-' * (bar_length - filled_len)

    sys.stdout.write('[%s] %s%s ...%s\n' % (bar, percents, '%', status_message))
    sys.stdout.flush()  # As suggested by Rom Ruben (see: http://stackoverflow.com/questions/3173320/text-progress-bar-in-the-console/27871113#comment50529068_27871113)
